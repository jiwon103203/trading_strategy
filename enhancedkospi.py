...
class EnhancedKospiSectorRSStrategy:
    def __init__(self, length=20, timeframe='daily', recent_cross_days=None):
        """
        개선된 코스피 섹터 RS 전략
        
        Parameters:
        - length: RS 계산 기간 (기본값: 20)
        - timeframe: 'daily' 또는 'weekly' (기본값: 'daily')
        - recent_cross_days: 최근 N일 내에 100을 넘긴 섹터만 선택 (None이면 비활성화)
        """
        # 코스피 섹터 TIGER ETF 목록
        self.sector_etfs = {
        ...
        # 벤치마크: KOSPI 200 ETF
        self.benchmark = '069500.KS'  # KODEX 200
        ...
        print(f"전략 초기화: timeframe={self.timeframe}, length={self.length}, recent_cross_days={self.recent_cross_days}")
    ...
    def check_recent_cross(self, rs_components, analysis_date, days_back):
        ...
        if rs_components.empty or days_back is None:
            return True  # 필터링 비활성화
        
        try:
            # 분석 날짜 기준으로 과거 데이터 선택
            end_date = analysis_date
            
            # 주봉인 경우 days_back을 주 단위로 변환
    ...
    def backtest(self, start_date, end_date, initial_capital=10000000):
        ...
        for i, rebal_date in enumerate(rebalance_dates):
            ...
            # 주봉인 경우 가장 가까운 주말 날짜로 조정
            if self.timeframe == 'weekly':
                # 해당 주의 금요일로 조정
                days_ahead = 4 - rebal_date.weekday()  # 금요일은 4
                if days_ahead < 0:
                    days_ahead += 7
